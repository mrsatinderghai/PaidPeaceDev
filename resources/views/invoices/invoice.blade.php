...
.twoBtn form:first-child {
  position: absolute;
  left: 50%;
  transform: translate(-100%, 0px);
}
.twoBtn form:last-child {
  position: absolute;
  left: 50%;
...
<div class="container">
<!-- <img height="100px" src="img/sharpmower.jpg" style='height:100px; width:100px' /><br /> -->
...
<div class="d-flex twoBtn">
  <form action="{{ url('invoice/send/'.$invoice->id) }}" method="POST" class="text-center">
      {{ csrf_field() }}
      {{ method_field('PATCH') }}
      <button class="btn btn-success"><i class="fa fa-check"></i> Send Invoice</button>
  </form>
  <form action="{{ url('invoice/resend/'.$invoice->id) }}" method="POST" class="text-center">
      {{ csrf_field() }}
      {{ method_field('PATCH') }}
      <button class="btn btn-primary"><i class="fa fa-send"></i> Resend Invoice</button>
  </form>  
</div>
</div>
@endsection
